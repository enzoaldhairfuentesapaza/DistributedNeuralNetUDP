...
from load_avg_gradient import load_average_gradients
...
for epoch in range(num_epochs):
    ...
    for batch_x, batch_y in train_loader:
        ...
        # --- DIVISION DEL BACKWARD--------------------------------------------------------
        os.makedirs("assignments", exist_ok=True)

        NUM_WORKERS = 2

        parts_x = np.array_split(batch_x.numpy(), NUM_WORKERS)
        parts_y = np.array_split(batch_y.numpy(), NUM_WORKERS)

        for worker_id in range(NUM_WORKERS):

            filename = f"assignments/worker_{worker_id+1}.bin"

            with open(filename, "wb") as f:

                np.save(f, parts_x[worker_id])
                np.save(f, parts_y[worker_id])

                np.save(f, model.fc1.weight.detach().numpy())
                np.save(f, model.fc1.bias.detach().numpy())

                np.save(f, model.fc2.weight.detach().numpy())
                np.save(f, model.fc2.bias.detach().numpy())

                np.save(f, model.class_logits.weight.detach().numpy())
                np.save(f, model.class_logits.bias.detach().numpy())
        # ----------------------------------------------------------------------------------
        input("\nPresiona ENTER cuando master.cpp termine...")
        load_average_gradients(model)
        optimizer.step()
