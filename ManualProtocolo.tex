...
\hline
ACK inicial sin paquetes válidos & ACK\_NONE = 0xFFFFFFFF \\
\hline
...
\item \textbf{Número ACK:} Indica el último número de secuencia recibido correctamente y en orden. Este campo se utiliza principalmente en paquetes ACK. Por ejemplo, si el receptor recibió correctamente los paquetes 0, 1 y 2, enviará un ACK con valor 2. Si todavía no recibió ningún DATA válido, utilizará el valor especial ACK\_NONE.
...
Cada objeto será serializado por la biblioteca C++ en una secuencia binaria. Luego el protocolo RDT fragmentará esa secuencia en paquetes DATA, los transmitirá usando Go-Back-N y reconstruirá exactamente el mismo objeto en el receptor. Una vez reconstruido, la capa de aplicación deserializará el objeto y procesará sus tensores internos.
...
\hline
START & Tipo de paquete, ID de transferencia, ID del emisor, ID del receptor, tipo de objeto, tamaño total del objeto, total de fragmentos, CRC32 \\
\hline
DATA & Tipo de paquete, número de secuencia, ID de transferencia, ID del emisor, ID del receptor, tipo de objeto, tamaño total del objeto, número de fragmento, total de fragmentos, tamaño útil del payload, CRC32, payload \\
\hline
ACK & Tipo de paquete, número ACK, ID de transferencia, ID del emisor, ID del receptor, CRC32 \\
\hline
END & Tipo de paquete, número de secuencia, número ACK, ID de transferencia, ID del emisor, ID del receptor, CRC32 \\
\hline
\end{longtable}
...
Esto permite detectar corrupción no solo en el contenido transmitido, sino también en campos críticos como el tipo de paquete, número de secuencia, número ACK, ID de transferencia, ID del emisor, ID del receptor, tipo de objeto, tamaño total del objeto, número de fragmento, total de fragmentos y tamaño útil del payload.
...
\item El receptor reenvía el ACK acumulativo del último paquete recibido correctamente en orden. Si todavía no recibió ningún DATA válido, responde con ACK\_NONE.
...
\item Un ACK con valor ACK\_NONE indica que todavía no se recibió ningún paquete DATA válido de la transferencia.
...
\item El emisor envía un paquete START.

\item El receptor responde con ACK.

\item El emisor comienza el envío continuo de paquetes DATA dentro de la ventana Go-Back-N.
...
\begin{itemize}
\item El receptor acepta el paquete.
...
\begin{itemize}
\item El receptor descarta el paquete.
\item El receptor reenvía un ACK acumulativo con el último número de secuencia recibido correctamente en orden.
\item Si todavía no existe un paquete válido confirmado, el receptor responde con ACK\_NONE.
\end{itemize}

\item Si un paquete llega corrupto:
\begin{itemize}
\item El receptor descarta el paquete completo.
...
\begin{itemize}
\item El emisor retransmite todos los paquetes desde el paquete posterior al último ACK confirmado.
\item Si el último ACK confirmado es ACK\_NONE, el emisor retransmite desde el paquete DATA con secuencia 0.
\end{itemize}
...
C++ convertirá cada objeto en una secuencia binaria para fragmentarla en paquetes UDP de hasta 512 bytes.
...
C++ no definirá la red neuronal. La biblioteca C++ se encargará de serializar objetos de aplicación, fragmentarlos, enviar y recibir paquetes UDP, aplicar Go-Back-N, verificar CRC32, administrar ACK acumulativos, reconstruir objetos completos y devolver hacia Python los gradientes contenidos en los objetos \texttt{GRADIENT\_RESULT}.
