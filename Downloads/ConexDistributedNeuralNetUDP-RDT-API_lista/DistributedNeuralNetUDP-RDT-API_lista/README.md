...
This project uses Python for model logic and C++ for the UDP RDT transport layer. The Python code calls the C++ transport API through a local binding.

## Build environment

Use a local Python virtual environment in WSL to avoid system-managed Python restrictions. From the project root:
...
If your WSL distribution does not include the Python development headers, install them with your package manager before building:
...
- This creates `api_rdt_udp.so` in the project directory.

- Example usage is in `python_api_example.py`.

- The Python interface exposes:
  - `MasterTransport`
  - `WorkerTransport`
  - `default_workers()`
